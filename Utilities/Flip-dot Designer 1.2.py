...
def dotEditor(numRows=112, numCols=16, comPort="COM_"):
    if comPort != "COM_":
    ...
    button = tk.Button(inside_frame, width=5, height=1, text="(R,C)", bg='grey', relief="flat")
...
def launchMenu():
    ...
    numCols.set(112)
    comLabel = tk.Label(menu, text='Serial Port (optional)', anchor="w", justify="left")
    comPortBox = tk.Entry(menu, textvariable=comPort, width=16, justify="left")
    comPort.set("COM_")
    ...
    quickButton = tk.Button(menu, width=16, height=1, text="Launch 8R x 8C", bg='grey',
                            command=lambda: [menu.destroy, dotEditor(8, 8, comPort.get())])
    quickButton2 = tk.Button(menu, width=16, height=1, text="Launch 16R x 112C", bg='grey',
                             command=lambda: [menu.destroy, dotEditor(16, 112, comPort.get())])
    quickButton3 = tk.Button(menu, width=16, height=1, text="Launch 7R x 90C", bg='grey',
                             command=lambda: [menu.destroy, dotEditor(7, 90, comPort.get())])
    quickButton4 = tk.Button(menu, width=16, height=1, text="Launch 7R x 30C", bg='grey',
                             command=lambda: [menu.destroy, dotEditor(7, 30, comPort.get())])
    launch = tk.Button(menu, width=16, height=1, text="Launch", bg='grey',
                       command=lambda: [menu.destroy, dotEditor(numRows.get(), numCols.get(), comPort.get())])
    ...
    comLabel.grid(row=4, column=0)
    comPortBox.grid(row=4, column=1)
    # importLabel.grid(row=2, column=0)
    # importBox.grid(row=2, column=1)
    launch.grid(row=5, column=1)
    emptyLabel1.grid(row=6, column=0)
    presetsLabel.grid(row=7, column=0)
    quickButton.grid(row=8, column=0)
    quickButton2.grid(row=8, column=1)
    quickButton3.grid(row=9, column=0)
    quickButton4.grid(row=9, column=1)
    # noteLabel.grid(row=8, column=0, columnspan=2)
    emptyLabel2.grid(row=10)
    emptyLabel3.grid(row=11)
    creatorLabel.grid(row=12, column=0, columnspan=2, rowspan=4)
